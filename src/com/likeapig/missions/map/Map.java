...
public class Map {
	...
	public void firstRound() {
		...
		for (NPC npc : npcs) {
			npc.getNavigator().setTarget(getPlayer(), true);
		}
	...
	public void removePlayer(final Player p) {
		if (this.containsPlayer(p)) {
			data.restore();
			if (this.state.equals(MapState.STARTING) || (this.state.equals(MapState.STARTED))) {
				this.stop();
	...
	public boolean containsPlayer(final Player p) {
		if (data.getMap() == this && data.getPlayer().equals(p)) {
			return true;
		}
		return false;
	}
	...
	public enum MapState {
		WAITING("WAITING", 0, "WAITING", 0, "WAITING", true), STARTING("STARTING", 1, "STARTING", 1, "STARTING",
				true), STARTED("STARTED", 2, "STARTED", 2, "STARTED",
						false), STOPPED("STOPPED", 3, "STOPPED", 3, "STOPPED", false);

		private boolean allowJoin;
		private String name;

		private MapState(final String s2, final int n2, final String s, final int n, final String name,
				final Boolean allowJoin) {
